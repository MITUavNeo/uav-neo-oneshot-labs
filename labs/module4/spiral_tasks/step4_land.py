...
def update(drone):
    ...
    if _phase == 0:   # centre over target using downward camera
        ...
        # Step 2: contours = rc_utils.find_contours(img, step2_fly_leg.TARGET_LOWER, step2_fly_leg.TARGET_UPPER)
        #         best = rc_utils.get_largest_contour(contours, 200)
        # Step 3: if best is None → drone.flight.stop(); return False
        # Step 4: cx, cy = rc_utils.get_contour_center(best)
