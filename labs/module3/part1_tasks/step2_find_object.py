...
Your task:
    ...
    4. Find the largest contour with rc_utils.get_largest_contour(..., MIN_AREA).
    5. If found: get centroid with rc_utils.get_contour_center(contour),
...
def update(drone):
    ...
    # Step 2: contours1 = rc_utils.find_contours(image, TARGET_LOWER,  TARGET_UPPER)
    #         contours2 = rc_utils.find_contours(image, TARGET_LOWER2, TARGET_UPPER2)
    #         all_contours = contours1 + contours2
    # Step 3: best = rc_utils.get_largest_contour(all_contours, MIN_AREA)
    # Step 4: if best is not None:
    #             cx, cy = rc_utils.get_contour_center(best)
